...
func (tree *tree) updateRootNode() {
	...
	for key, node := range tree.nodeTable {
		_, prs := allChildren[key]
		if !prs {
			tree.Root = node
			break
		}
...
func parseLine(line string) (name string, weight int, children []string, err error) {
	split := strings.Split(strings.Replace(line, ", ", ",", -1), " ")
	name = split[0]
	weight, err = strconv.Atoi(strings.Trim(split[1], "()"))
	if len(split) > 2 {
		children = strings.Split(split[3], ",")
	}
...
func lightest(is []weightAndNode) weightAndNode {
	min := weightAndNode{weight: math.MaxInt16}
	for _, e := range is {
...
func equalWeight(is []weightAndNode) bool {
	prev := is[0]
	for _, e := range is {
...
func findOverweightNode(node *node) (*node, int) {
	...
	childWeights := calculateWeights(node.Children)

	// If all nodes are equal the current node is the fat one
	if equalWeight(childWeights) {
		return node, 0
	}
...
func createTree(input string) *tree {
	...
	for scanner.Scan() {
		...
		if err != nil {
			fmt.Fprintln(os.Stderr, "parsing line: ", scanner.Text(), err)
			continue
		}
		tree.add(name, weight, children)
	}
	if err := scanner.Err(); err != nil {
		fmt.Fprintln(os.Stderr, "reading input data: ", err)
	}
